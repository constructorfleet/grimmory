...
ALTER TABLE book_additional_file RENAME TO book_file_old;
CREATE TABLE book_file (LIKE book_file_old INCLUDING ALL);
...
-- Regenerate generated column for alternative book format files, create the index without UNIQUE constraint
ALTER TABLE book_file ADD COLUMN alt_format_current_hash VARCHAR(128) GENERATED ALWAYS AS (CASE WHEN is_book THEN current_hash END) STORED;
CREATE INDEX idx_book_file_current_hash_alt_format ON book_file (alt_format_current_hash);

-- Remove constraint from book table
DROP INDEX IF EXISTS unique_library_file_path;
